...
public class OsmDataTests
{
    ...
    public async Task Land_reprojects_from_mercator_to_wgs84()
    {
        ...
        var land = new FeatureCollection { MercatorSquare(0, 0, 1) };
        var osm = new OsmData(land, []);

        var result = osm.Land(new Envelope(-1, -1, 1, 1));
    ...
    public async Task Land_drops_features_disjoint_from_bounds()
    {
        ...
        var land = new FeatureCollection { MercatorSquare(10_000_000, 0, 100_000) };
        var osm = new OsmData(land, []);
        var result = osm.Land(new Envelope(-1, -1, 1, 1));
    ...
    {
        var land = new FeatureCollection { MercatorSquare(0, 0, 1) };
        var ocean = new FeatureCollection { MercatorSquare(0, 0, 2) };
        var osm = new OsmData(land, ocean);

        var landResult = osm.Land(new Envelope(-1, -1, 1, 1));
        var oceanResult = osm.Ocean(new Envelope(-1, -1, 1, 1));
    ...
    public async Task Coastline_emits_line_strings_from_polygon_outlines()
    {
        ...
        var land = new FeatureCollection { MercatorSquare(0, 0, 1) };
        var osm = new OsmData(land, []);
        var lines = osm.Coastline(new Envelope(-1, -1, 1, 1));
        await Assert.That(lines.Count).IsEqualTo(1);
        var type = lines[0].Geometry!.Type;
        await Assert.That(type == GeometryType.LineString || type == GeometryType.MultiLineString).IsTrue();
    }
}
